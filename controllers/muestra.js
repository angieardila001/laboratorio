...
const muestraGet = async (req, res) => { //listar todos
  try {
    const muestras = await Muestra.find().populate({ path: "solicitante", populate: { path: "ciudad" } }).populate({ path: "munRecoleccion" }).populate({ path: "tipoMuestra" }).populate({ path: "cotizacion" })
